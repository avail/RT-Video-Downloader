...
def getLinks(videoName, url): # This function takes the name of a video and a url and gets to work fetching links.
    ...
    chosenRes = validInput('Which resolution would you like to download?\nAvaliable resolutions:', resolutions, True) # Makes sure the input is only of the featured resolutions.
...
def removeComments(playlist): # This function takesvalidInput a raw playlist and returns it as a list, removing lines that are empty and start with a hash.
...
def validInput(prompt, options, default = False): # This function takes a prompt and a dictionary of options and validates the input.
    goodInput = False
    string = prompt + '\n'
    ...
    while not goodInput:
        ...
        if default == True and userInput == "": # if input was blank, default to 1080p
            return "NewHLS-1080"
